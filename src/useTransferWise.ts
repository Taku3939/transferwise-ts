import { CreateQuoteRequest, CreateQuoteResponse, RecipentAccountRequest, RecipentAccountResponse, CreateTransferRequest, CreateTransferResponse, FundTransferRequest, FundTransferResponse, GetProfileResponse, TransferDeliveryTimeResponse } from '.'
...
export type TransferWiseClient = {
  ...
  createRecipientAccount(request: RecipentAccountRequest): Promise<RecipentAccountResponse>
  createTransfer(request: CreateTransferRequest): Promise<CreateTransferResponse>
  fundTransfer(profileId: number, transferId: number, request: FundTransferRequest): Promise<FundTransferResponse>
  getTransferDeliveryTime(transferId: number): Promise<TransferDeliveryTimeResponse>
}
...
export const useTransferWise = (config: TransferWiseConfig): TransferWiseClient => {
  const urls = {
   ...
   * @returns
   */
  const createRecipientAccount = async (request: RecipentAccountRequest): Promise<RecipentAccountResponse> => {
   ...
   * @returns
   */
  const fundTransfer = async (profileId: number, transferId: number, request: FundTransferRequest): Promise<FundTransferResponse> => {
    const fetchData = await fetch(`${baseUrl}/v3/profiles/${profileId}/transfers/${transferId}/payments`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${config.apiKey}`,
        'Content-Type': 'application/json',
   ...
   */
  const getTransferDeliveryTime = async (transferId: number): Promise<TransferDeliveryTimeResponse> => {
    const fetchData = await fetch(`${baseUrl}/v1/delivery-estimates/${transferId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${config.apiKey}`,
      },
